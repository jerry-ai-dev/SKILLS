...
### Q4【选择，5分】
以下关于 **DPO** 的说法，哪一项是**错误**的？

A. DPO 跳过了单独训练 Reward Model 这一步，直接用偏好对 $(x, y_w, y_l)$ 训练策略  
B. DPO 仍然需要参考模型 $\pi_{\text{ref}}$（一般是 SFT 检查点）作为 KL 锚  
C. DPO 的隐式 reward 定义为 $\hat r_\theta(x,y) = \beta\log(\pi_\theta(y|x)/\pi_{\text{ref}}(y|x))$  
D. 由于 DPO 是离线监督，它能像 GRPO 一样让模型涌现出训练数据中从未出现的反思 / "Aha moment" 行为  

**答案**：D。DPO 只能强化偏好集中已有的模式，看不到自己采样的新轨迹，难以涌现新行为。R1 的 Aha moment 依赖在线 RL 探索（GRPO），不是 DPO 能做到的。
